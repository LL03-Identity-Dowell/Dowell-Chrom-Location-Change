...
if __name__ == '__main__':
    ...
    # Get latitude and longitude from the spreadsheet
    ...
    # Create instances of GeoSearchView for each combination
    list_of_objects = [GeoSearchView() for _ in range(len(combinations))]
    ...
    # Insert a new MongoDB document and get its Object ID
    object_id = collection_name.insert_one(rec).inserted_id

    # Use multi-threading to execute the write function for each combination
    pool.starmap(GeoSearchView.write, zip(list_of_numbers, list_of_objects, combinations,
                                           repeat(latitude, len(combinations)), repeat(longitude, len(combinations)),
                                           repeat(object_id, len(combinations)), repeat(city, len(combinations)),
                                           repeat(language_dataframe, len(combinations))))

    # Close the thread pool
    pool.close()

    # Synchronize and handle exceptions
    pool.join()
